...
class ApiClient {
    ...
    getComment(params: { commentId: number, jwt?: string }): Promise<CommentResponse> {
        return this.wrapForApiTimeouts(this.innerClient.getComment({
            id: params.commentId,
            auth: params.jwt,
        }));
    }
    ...
    editComment(params: { commentId: number, content: string, jwt: string }): Promise<CommentResponse> {
        return this.wrapForApiTimeouts(this.innerClient.editComment({
    ...
    search(params: { query: string, jwt?: string }): Promise<SearchResponse> {
        return this.wrapForApiTimeouts(this.innerClient.search({
            q: params.query,
            auth: params.jwt,
        }));
    }
    ...
    setSubscriptionStatus(params: { communityId: number, status: boolean, jwt: string }) {
        return this.wrapForApiTimeouts(this.innerClient.followCommunity({
    ...
    getUnreadCount(params: { jwt: string }) {
        return this.wrapForApiTimeouts(this.innerClient.getUnreadCount({
            auth: params.jwt,
        }))
    }

    getReplies(params: { unreadOnly: boolean, jwt: string }) {
        return this.wrapForApiTimeouts(this.innerClient.getReplies({
    ...
    setCommentReadStatus(params: { commentReplyId: number, readStatus: boolean, jwt: string }) {
        return this.wrapForApiTimeouts(this.innerClient.markCommentReplyAsRead({
